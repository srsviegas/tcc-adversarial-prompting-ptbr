...
def print_header(model_path: str, instruction_key: str, system_prompt: str, lang: str):
    if HAS_RICH:
        ...
        console.print(Panel(grid, title="[bold magenta]Gemma-2-27B-it-Abliterated Direct Prompting[/bold magenta]", border_style="cyan"))
    else:
        print("=" * 70)
        print(" Gemma-2-27B-it-Abliterated Direct Prompting")
        print("=" * 70)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Prompt Gemma 2 27B Instruct Abliterated GGUF directly using selected AI Instructions",
...
Examples:
  ...
  py scripts/prompt_gemma_2_27b_abliterated.py "Quais são os principais vetores de injeção de prompt?"

  # 2. Single prompt with internetês caótico / shitpost translator:
  py scripts/prompt_gemma_2_27b_abliterated.py -i internetes "O projeto foi finalizado dentro do prazo estipulado."

  # 3. Single prompt with Base64 exclusive protocol instruction:
  py scripts/prompt_gemma_2_27b_abliterated.py -i base64 "Explique o algoritmo Diffie-Hellman."

  # 4. Interactive chat mode with Fraktur stylized alphabet instruction:
  py scripts/prompt_gemma_2_27b_abliterated.py -i fraktur -it

  # 5. Interactive chat mode with Caesar cipher (shift 5):
  py scripts/prompt_gemma_2_27b_abliterated.py -i caesar --shift 5 -it
        """
    )
    parser.add_argument(
        "prompt",
