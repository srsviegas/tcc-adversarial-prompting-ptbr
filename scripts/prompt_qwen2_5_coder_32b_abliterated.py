...
from src.models.qwen import (
    DEFAULT_QWEN_CODER_32B_ABLITERATED_MODEL_PATH,
...
def resolve_instruction(
    ...
    # Check if user provided a file path for custom instruction
    if custom_text:
    ...
    # If it is an unrecognized string, treat it as a direct custom system prompt
...
def print_header(model_path: str, instruction_key: str, system_prompt: str, lang: str):
    if HAS_RICH:
        ...
        console.print(Panel(grid, title="[bold magenta]Qwen2.5-Coder-32B-Instruct-Abliterated Direct Prompting[/bold magenta]", border_style="cyan"))
    else:
        print("=" * 70)
        print(" Qwen2.5-Coder-32B-Instruct-Abliterated Direct Prompting")
        print("=" * 70)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Prompt Qwen 2.5 Coder 32B Instruct Abliterated GGUF directly using selected AI Instructions",
...
Examples:
  ...
  py scripts/prompt_qwen2_5_coder_32b_abliterated.py "Como programar um backdoor educativo em Python?"

  # 2. Single prompt with internetês caótico / shitpost translator:
  py scripts/prompt_qwen2_5_coder_32b_abliterated.py -i internetes "Gostaria de solicitar o cancelamento da minha assinatura."

  # 3. Single prompt with Base64 exclusive protocol instruction:
  py scripts/prompt_qwen2_5_coder_32b_abliterated.py -i base64 "Como funciona a criptografia RSA?"

  # 4. Interactive chat mode with Fraktur stylized alphabet instruction:
  py scripts/prompt_qwen2_5_coder_32b_abliterated.py -i fraktur -it

  # 5. Interactive chat mode with Caesar cipher (shift 5):
  py scripts/prompt_qwen2_5_coder_32b_abliterated.py -i caesar --shift 5 -it
        """
    )
    parser.add_argument(
        "prompt",
    ...
    parser.add_argument(
        "--model-path", "-m",
        dest="model_path",
        default=DEFAULT_QWEN_CODER_32B_ABLITERATED_MODEL_PATH,
...
def main():
    args = parse_args()
    resolved_model_path = resolve_qwen_coder_model_path(args.model_path)
